...
def live_xy(x_vec, y1_data, line1, identifier='', pause_time=1):
    ...
    if line1 == []:
        ...
        line1, = ax.plot(x_vec, y1_data, '-o', alpha=0.8, color='green',markerfacecolor='yellow',label='磷化氢')
        plt.legend(loc="upper left", shadow=True)
        plt.ylabel('占空气百分比')
        plt.title('PH3浓度{}'.format(identifier))
        plt.show()
    if data_n < 20:
        ...
        plt.clf()
        line1, = plt.plot(x_vec, y1_data, '-o', alpha=0.8, color='green', markerfacecolor='yellow', label='磷化氢')
        for a, b in zip(x_vec, y1_data):  # 标点数据
            plt.text(a, b, b, ha='center', va='bottom', fontsize=10)  # markerfacecolor='orange'标点颜色
        ...
        y1_data1=datain.dataset2[data_n-20:data_n]
...
while True:
    ...
    if data_n < 20:
        ...
        #     x.append(i)
        x_vec = x
        #y_vec = datain.dataset2[0:len(x)]#20个

        rand_val = datain.dataset2[data_n]#第21个数
