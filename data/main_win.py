...
def lb_gettime():#标签时间
    global a2
    s = 'ABCDEF0123456789'  #'#800000' 颜色码
    a2 = '#'
    a1 = []
    for j in range(0, 6):
        a1.append(random.choice(s))#随机选择s的一个字符 添加到 a1末尾中
        a2 = a2 + a1[j]     #添加 当前添加的字符 在末尾
    timestr=time.strftime('%Y/%m/%d %H:%M:%S')
    lb_time.configure(text=timestr,fg=a2)
    w.after(1000, lb_gettime) #每1000毫秒执行一次getime()函数
...
mb3.add_cascade(label='流氓表白神器',command=_img_mb_test)
